...
(function($,window){		
	...
	$.widget("mobile.multiview",$.mobile.widget, {
		...
		_setupSplitview: function() {
			...
			// need to setup splitview like this to ensure it fires when the page is loaded directly OR pulled into the DOM of an already existing page
				...
				function flagIt() {
					...
					// set a flag
					$(this).data("enhanced", "no");
					console.log("flag set to ="+$(this).data("enhanced") );
					$(this).die('pageinit', flagIt);
					}
					
				// this sets up the flag for firing the next function only once per trigger element
				$( 'div:jqmData(role="page"):jqmData(wrapper="true")' ).live( 'pageinit',flagIt);
				*/
				...
				// this should only fire once per wrapper-page
				// $( 'div:jqmData(role="page"):jqmData(wrapper="true")' ).live( 'pagebeforeshow',function(event){
					
						
						// set a flag
						// $(this).data("enhanced", "no");
						// console.log("wrapper pagebeforeshow enhanced="+$(this).data("enhanced")+"  id="+$(this).attr('id') );
					
						// make sure this fires once per trigger element and not on every changePage
						// if ( $(this).data("enhanced") == "no" ) {
												
							// flag it
							// $(this).data("enhanced", "yes");
							
							// console.log( "nachher="+$(this).data("enhanced") );
							...
							$(':jqmData(wrapper="true") :jqmData(role="page")').attr('data-internal-page', 'true');	
							//enhance first pages on all panels		
							// TODO: trigger('create/pagecreate'), doesn't work on pages
							// because of using page, I need to block the plugin trigger auto-firing						
							var firstPage=$('div:jqmData(role="panel") div:jqmData(show="first")')
									.addClass('ui-page '+$.mobile.activePageClass)
									.page();	
									//.trigger("create");
							...
							firstPage.each(function() {															
								...
								// grab options of wrapper page, as this page doesn't have a data("page")								
								var options = $(this).closest(':jqmData(wrapper="true")').data("page").options;
								
								// make settings
								var settings = $.extend( {}, $.mobile.changePage.defaults, options);
								//var mySettings = $.extend( {}, $.mobile.loadPage.defaults, options);
								var triggerData = {element: $('#'+toPage), options: settings };
								
								console.log( [triggerData] );
								// $.mobile.transitionpages.enhancePage( $(this) );
								triggerData.element.page();
								});
							*/
							...
							if ( $("div:jqmData(wrapper)").length  ) {
								console.log("wrapper detected, either menua/main or fullwidth layout");
		...
		// --------------------------- end JQM copy ------------------------ 
		/*	
		// this is necessary, because data-wrapper is not accessible from a 
		// clicked link, so you cannot determine if a page to be loaded in 
		// should be in splitview or not. Pageshow ensures that any formatting 
		// also can be re-applied in case an initially loaded splitview page is  
		// left (clears formatting) and user goes back to it later on, whereas
		// pagecreate wouldn't fire here
		$( ':jqmData(role="page")' ).live( 'pagebeforeshow',function(event){
				// if the page to be loaded has the triggering data-wrapper
				if ( $(this).jqmData("wrapper") ) {
...
*/
			...
			// data-multiview is needed, because if an external page is pulled via Ajax, I only have href (page2.html) 					
			if (typeof $link.jqmData('target') == 'undefined' && !$link.jqmData('wrapper') ) {
				console.log("splitview not detected, start cleaning up");
				//wait a little, then clear all splitview/popover formatting 
				window.setTimeout( function() {
					$('html').removeClass('ui-multiview-active ui-splitview-mode ui-popover-mode switchable');
					$('div:jqmData(panel="main")').removeClass( 'ui-panel-right ui-border-right');
		...
		splitScreen: function( event ) {	
			...
			// setup popover mode
			function popover() {				
				if ( self.vars.$html.hasClass('ui-popover-mode') || self.vars.$html.is('ui-fullscreen-mode') ) {															
					return;
					}
			...
			function splitView () {				
				if ( self.vars.$html.hasClass('ui-splitview-mode') || self.vars.$html.is('ui-fullscreen-mode') ) {										
					return;
					}
				console.log("in splitview");
				var $menu=$('div:jqmData(id="menu")'),
					$main=$('div:jqmData(id="main")'),
		...
		*/
		...
		stackUp: function (source, event, data) {			
				...
				// console.log("stackUp fired from "+source);
		...
		stackDown: function ( source, whereToGo ) {
			...
			// console.log("stackDown fired from "+source);
		...
		_mainEventBindings: function () {
			...
			$(document).bind( "click", function( event ) {
				// check for splitivew on clicked links
...
var trigger = $('div:jqmData(wrapper="true")').live( 'pagecreate',function(event){ 
	// the lock is necessary, because page() is used twice in the plugin which causes this to fire 
	// multiple time. Using trigger('create/pagecreate') it fires only once, but then panel-pages
	// are not enhanced. Using trigger('create'), from.data("page") is undefined, using 
	// trigger('pagecreate'), to.page("data") is undefined on the first transition
