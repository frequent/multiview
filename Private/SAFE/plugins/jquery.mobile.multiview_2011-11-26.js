...
// TODOS:		
		...
		// Find another solution to position triangles - triangel is gone in FF...
		// make triangle show up on every page vs. first page only
		...
		// ==== GEN PRIO ====
		// activeClass on buttons after backward transition 
		...
		// reset URL path to main/fullwidth data-show="first" on hideAllPanels(), changePage to data-show="first"!	 window.location.hash = 'hash-value' to update URL
		...
		// Panel height is not adapted to background= longest active page on a panel		
		...
		// back button on external pages added to DOM does not work
...
(function( $, window) {	
	...
	$.widget("mobile.multiview",$.mobile.widget, {
		...
		// TODO: what's faster, global definition or defining within each function?
		vars: {		

			// reference shorts
			$wrapper: $(':jqmData(wrapper="true")'),
			$main : $(":jqmData(panel='main')"),
			$menu : $(":jqmData(panel='menu')"),
			$full : $(":jqmData(panel='fullwidth')"),
			$first: $(":jqmData(show='first')"),
			$header: $(":jqmData(role='header')"),
			$footer: $(":jqmData(role='footer')"),
			$content: $(":jqmData(role='content')"),
			$panel: $(":jqmData(role='panel')"),
			$popPanel: $(":jqmData(panel='popover')"),
			$page: $(":jqmData(role='page')"),
			
			// screenmode shorts
			$fullscreen: $('html').hasClass('ui-fullscreen-mode'),
			$splitiview: $('html').hasClass('ui-splitview-mode'),
			
			// panel shorts			
			$backPanels : $(":jqmData(panel='main'), :jqmData(panel='fullwidth')" ),
			$popovers : $(":jqmData(panel='popover'), .ui-popover-mode :jqmData(panel='menu')"),
		...
		_create: function() {		
			...
			$(":jqmData(role='panel')").addClass('ui-mobile-viewport ui-panel');
			
			// needed here (popovers, main/menu will be done by splitview setup)
			$(":jqmData(panel='fullwidth')").addClass('ui-panel-active');

			// add global header/footer classes
			if ( ':jqmData(wrapper="true") :jqmData(role="panel")').length ) {
				$(':jqmData(wrapper="true")').children(':jqmData(role="header")').addClass('ui-header-global');
				$(':jqmData(wrapper="true")').children(':jqmData(role="footer")').addClass('ui-footer-global');				
				}			
			
			// default set history to off
			$(':jqmData(hash="history")').data("history","off");
			...
			// setup
			// commented out because it also fires from _mainEvents pageshow binding (avoid calling twice)			
		...
		setupSplitview: function(event) {
			...
			var self = this,
				...
				firstPage=$(':jqmData(role="panel") > :jqmData(show="first")')
								.addClass($.mobile.activePageClass).trigger('create').page();	
			
			// add internal page status on all panel pages to enable panel-cache
			self.vars.$wrapper.find( self.vars.$page ).attr('data-internal-page', 'true');
			  
			
			
			// if panels, fire splitscreen
			if ( self.vars.$main.length ||  self.vars.$menu.length || self.vars.$full.length ) {
		...
		_setupPopovers: function() {
			...
			$(":jqmData(panel='popover'), .ui-popover-mode :jqmData(panel='menu')").each(function(index) {
			...
			$( self.vars.$backPanels ).live('scrollstart', function() {							
				hideAllPanels();
				});
			...
			$(document).scroll(function(){
				...
				// appended to the DOM				     
				if ( !self.vars.$fullscreen && self.vars.$panelTransBlockScrollTop == false ) {										
					hideAllPanels();
					// reset for next;
			...
			$( self.vars.$backPanels ).live('click tap', function(event) {
			...
			$( self.vars.$backPanels ).live('pagebeforehide', function(event) {												
				hideAllPanels();
				});

			// (6) click of a link on a panel, which loads a page on another panel
			$('a').live('click', function () {								
				if ( self.vars.$fullscreen && $(this).jqmData('target') 
			...
			function hideAllPanels () {
				...
				$(":jqmData(panel='popover'), .ui-popover-mode :jqmData(panel='menu')").each(function(index) {		
					...
					window.setTimeout( function() {						
						$( self.vars.$panel ).removeClass('reverse out pop');
						}, 350);
			...
			$('.toggle_popover').live('click', function(event) {
				...
				var $correspond = $(this).jqmData("panel"),
					$popPanel = $(':jqmData(id="'+$correspond+'")');	
				...
				if ( $(this).jqmData('panel') == "menu" && $('html').hasClass('switchable') ) {														
					var $tweak = $( self.vars.$main ).find( self.vars.$header, self.vars.$content );					
				...
				if ( $popPanel.attr('status') == 'visible' ) {
					...
					hideAllPanels();
						...
						if ( self.vars.$fullscreen ) {							
							...
							$('.ui-page-active')
								.not( ":jqmData(wrapper='true'), :jqmData(id='"+$correspond+"') .ui-page-active" )
								.addClass("reActivate")
								.removeClass('ui-page-active');							
							}
		...
		crumble: function(event, data, page) {			
				...
				var $prevPage,
					onPage = $( '#'+page.attr('id') ),
					$crumPanel = $( '#'+page.attr('id') ).closest( self.vars.$panel ),
					$header = onPage.find( self.vars.$header ),
			  ...
			  function crumbify(button, href, text, page){				 				
					...
					var targetID = $( '#'+page.attr('id') ).closest( self.vars.$panel ).jqmData("id");					
					...
					if(!button.length || button.hasClass('ui-crumbs') == true ) {
							...
							// make new one 
							page.find( self.vars.$header ).prepend('<a class="ui-crumbs ui-btn-left" data-icon="arrow-l"></a>');														
		...
		replaceBackBtn: function() {														
				...
				self.vars.$main.find( self.vars.$page ).each(function(index) {

						var $header = $(this).find( self.vars.$header ),
		...
		splitScreen: function( event ) {	
			...
			var self = this,
				$window = $(window),
				$mainHeader = self.vars.$main.find('div.'+$.mobile.activePageClass+'> div:jqmData(role="header")');
			...
			// setup popover mode
			function popover() {				
				if ( $('html').hasClass('ui-popover-mode') || self.vars.$fullscreen ) {															
					return;
					}
							
				self.vars.$menu.addClass('ui-popover pop_menuBox ui-triangle-top ui-panel-active ui-element-fixed-top')
				...
				if(self.vars.$menu.hasClass('ui-triangle-top') ){					
					self.vars.$menu.prepend('<div class="popover_triangle"></div>');
					}
				self.vars.$main.removeClass('ui-panel-right')
						.css('width', '')
				...
				if( !self.vars.$fullscreen) {					
					self.vars.$popovers.addClass('ui-popover').removeClass('pop_fullscreen');					
					} else {	
						self.vars.$popovers.addClass('pop_fullscreen').removeClass('ui-popover');						
						}									
				popoverBtn("plain");				
				}
			...
			function splitView () {					
				if ( self.vars.$splitview || self.vars.$fullscreen ) {														
					return;
					}
					
				self.vars.$menu.removeClass('ui-popover pop_menuBox ui-triangle-top ui-panel-visible ui-element-fixed-top')
						.addClass('ui-panel-left ui-panel-active ui-panel-border-right')
						.removeAttr('status')
						.css({'width':'25%', 'min-width':'250px', 'display':''});				
				self.vars.$menu.children('.popover_triangle').remove();				
				self.vars.$menu.find('div:jqmData(show="first") .closePanel').remove();
				self.vars.$main.addClass('ui-panel-right ui-panel-active');							
				self.vars.$popPanel.removeClass('pop_fullscreen').addClass('ui-popover');							
		...
		scrollMe: function ( panel ) {	
					...
					if ($.support.touch && $page.data('scrollable', 'Off')) {
						self.vars.$page.data('scrollable', 'On');
						self.vars.$page.find('div[data-role="content"]').attr('data-scroll', 'y');
						self.vars.$page.find("[data-scroll]:not(.ui-scrollview-clip)").each(function() {
							var $this = $(this);						
		...
		gulliver: function() {
			...
			self.vars.$popPanel.each(function(){					
			...
			if ( self.framer() == "small" || maxHeight > $(window).height() ) {																
				...
				self.vars.$popovers.removeClass('ui-triangle-top ui-triangel-bottom ui-popover ui-popover-embedded').addClass('pop_fullscreen')
			...
			if ( !self.vars.$fullscreen  && $("html").hasClass('touch') ) {								
					self.scrollMe( $allPanels );			
				} 					
			

			self.vars.$popovers.each(function(index) {	
				...
				if ( $(this).find('.back_popover').length == 0 ) {
					...
					var $closeFirstPage = ( $(this).hasClass('pop_fullscreen') ) ? 'back' : 'close',
						...
						$firstPage = $(this).find(':jqmData(show="first")').not('.ui-splitview-mode :jqmData(panel="menu") :jqmData(role="page")');
						
					//TODO: do I need to page() again?
					$firstPage.page();
					$firstPage.find(':jqmData(role="header") h1').before($backButton);					
					$(this).find(':jqmData(show="first")').page();
		...
		checkWidth: function( fromWhere ) {
			var self = this;		
			// adjusts width of ui-content, because 15px padding somehow always messes things up - DON'T TOUCH! 

			// in splitview mode.			
			if (self.framer() != 'small') {
										
				var $tweakContent = self.vars.$main.find( self.vars.$content ),
					$tweakHeader =  self.vars.$main.find( self.vars.$header );
					
				if ( $( self.vars.$menu ).width() == '250' ) {
					...
					if ( $('html').hasClass('ui-multiview-active') ) {
						// and SPLITVIEW MODE, tweak content, make scrollbars visible
						if ( self.vars.$splitview ) {								
		...
		*/
		...
		expandHeight: function (masterPage, action) {
			...
			if ( self.vars.$fullscreen ) {				
				if (action == "toggle") {
						...
						var $popHeight = masterPage.find( self.vars.$content ).outerHeight(),
						...
						self.vars.$backPanels.find('.ui-page-active :jqmData(role="content")')
											.addClass('ui-expand')
											.css({'height': $benchMarkHeight-30, 'min-height':'50%','overflow-y':'hidden'});									
					} else {
		...
		stackDown: function ( source, whereToGo ) {
			...
			var	$closestPanel = $( '#'+whereToGo).closest( self.vars.$panel ),							
		...
		clearActiveClasses: function ( trigger, useBruteForce, toPage, fromPage, link ) {						
				...
				if (link) {
					...
					link.closest( self.vars.$page ).find('.ui-btn-active').not('.ui-clicked-me').removeClass('ui-btn-active');
					link.closest('.ui-btn').removeClass('ui-clicked-me');
					}

				// clear active links if to and from page are on the same panel
				if (toPage.closest( self.vars.$panel).jqmData("id") == fromPage.closest( self.vars.$panel ).jqmData("id")  ) {												
						// show some color
						window.setTimeout( function() {							
				...
				if ($targetPanelActivePage.attr('data-url') == url || $currPanelActivePage.attr('data-url') == url) {				
					...
					//if link refers to a page on another panel, changePage on that panel
						...
						//if link refers to a page inside the same panel, changePage on that panel
								...
								// TODO: still needed?
								$.mobile.activePage=$(':jqmData(id="main") > div.'+$.mobile.activePageClass+', :jqmData(id="fullwidth") > .'+$.mobile.activePageClass);
								}																														
		...
		_mainEventBindings: function () {
			...
			$( self.vars.$page ).live('pagebeforeshow', function(event, data){							
