...
// TODOS:		
		...
		// ==== MISC ==== 				
		// prefetching, form submit, dialog probably won't work												
		// make options		
		// make menu & main into data-sticky-panel="true", instead of referencing them by name all the time				
		// rework content height, completely broken - increase menu content min-height to page-height-toolbars, so scrollview runs full page height
		...
		// orientation-change multiplies menu button in fullscreen-mode
		// repair scrolling (seems fixed now)	
		// checkwidth doesn't work if menu width < 250px... 
		// integrate removal of active classes after reverse transitions
		// try only CSS-ing panels and leaving pages untouched vs. now tampering with both panels and pages
		...
		// find pushState solution for page1>page2(ext)>ext1.html(ext) = loading an external page into an external wrapper. All other cases work
		...
		// ==== FOOTER PRIO ====
		// footer is jumping up on non-multiview pages, probably cause it's not fixed or global, so regular footer, still has the same bug...
...
(function( $, window) {	
	...
	$.widget("mobile.multiview",$.mobile.widget, {
		...
		checkWidth: function( fromWhere ) {
			...
			// TODO: I don't like this anymore. Live without?
			
			if (self.framer() != 'small') {
												
				var $tweakContent = $('div:jqmData(panel="main") div:jqmData(role="page") div:jqmData(role="content")'),
					$tweakHeader =  $('div:jqmData(panel="main") div:jqmData(role="page") div:jqmData(role="header")');																								
					
				if ( $('div:jqmData(panel="menu")').width() == '250' ) {
					
					// IMPROVE - override content 15px JQM-CSS
					var $delta =  $('div:jqmData(panel="menu")').outerWidth()-Math.round(parseFloat($('div:jqmData(panel="main")').css('margin-left'))),
						$newWidth = $(window).width()-250,
					...
					if ( $('html').hasClass('ui-multiview-active') ) {
						...
						if ( $('html').hasClass('ui-splitview-mode') ) {								
							...
							$tweakContent.css({'margin-left':$delta}).width( $newWidth-$oddPaddingWidth );
							$tweakHeader.css({'margin-left':$delta}).width( $newWidth );;
							} else {
		...
		panelTrans: function (event) {
			...
			if ($targetPanel) {
				console.log("targetPanel identified!, erstmal preventDefault!");
				// stop JQM - 
