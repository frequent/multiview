...
// TODOS:		
		...
		// make options		
		// add switchable as option
		// add autoshow for popovers, to show always or once
		// do basic popover layouts, topleft, topright, bottomleft, bottomright
		// add option or class to drop menu button somewhere else, for example into subHeader or any other jqmData(dropat)
		// ui-header-global is not subtracted in panelHeight!
		// make menuToggle Icon optionable
		// popover menu button - check for all existing buttons, not only back button, also check for stuff that looks like a button, e.g. select box!
		...
		// dialogs won't work because of history/back button
		// JQM wrapper content with popover > JQM breaks layout, because line 974 $closestPanel.data("stack") is undefined!
		// global header does not calculate correctly, because it grabs all global headers. Make sure global is only assigned to header/footer inside the wrapper, not inside the page and that only the first child gets deducted from screen height
		// fix content overflow in JQM pages with panel, should be overflow-x: visible
		...
		// dialogs won't close because lin1492 - longest[0][longestLen - 2] is undefined, fix hash-Navigation
		...
		// criss crossing pages errors line 974 $closestPanel.data("stack") is undefined
