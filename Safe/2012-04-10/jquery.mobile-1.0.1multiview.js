...
/* exposed $.mobile methods	 */
	...
	$.mobile.changePage = function( toPage, options ) {
		...
		var mpc = settings.pageContainer,
			...
			triggerData = { toPage: toPage ? toPage : $( $.mobile.firstPage ), options: settings };
		...
		if ( typeof toPage == "string" ) {
			...
			$.mobile.loadPage( toPage, settings )
				...
				// XXX FREQUENT: in case of a deeplink-backwards panel transition
				// with the page to be loaded already in the DOM (should happen a 
				// lot, as closing panels' last visible pages can remain in the URL)
				// this makes sure the parent page is loaded and triggers a deeplink
				// transition inside multiview.			
				if ( newPage.parents('div:jqmData(wrapper="true")').length && newPage.parents('div:jqmData(wrapper="true").ui-page-active').length == 0) {																					
					$('html').data("multiviewDeeplink", '#'+newPage.attr('id') );										
					newPage = newPage.parents('div:jqmData(wrapper="true")');
					}										
		...
		$( document ).delegate( "form", "submit", function( event ) {
			...
			$.mobile.changePage(
				url,
				{
					...
					// XXX FREQUENT - added option pageContainer, override reloadPage
					reloadPage: false,			
					pageContainer: $this.jqmData('panel') ? $('div:jqmData(id="'+$this.jqmData('panel')+'")') : $.mobile.pageContainer
				}
			);
			event.preventDefault();
		});
		...
		$( document ).bind( "click", function( event ) {
			...
			var transition = $link.jqmData( "transition" ),
				...
				reverse = ( direction && direction === "reverse" ) ||
							...
							$link.jqmData( "back" ),
				// XXX FREQUENT: assigning pageContainer and passing it along to changePage
				pageContainer = $link.jqmData('panel') ? $('div:jqmData(id="'+$link.jqmData('panel')+'")') : $.mobile.pageContainer,
				
				//this may need to be more specific as we use data-rel more
				role = $link.attr( "data-" + $.mobile.ns + "rel" ) || undefined;
						
			$.mobile.changePage( href, { transition: transition, reverse: reverse, role: role, pageContainer:pageContainer } );
			event.preventDefault();
			});
			...
			if ( to ) {				
				...
				if (!$panels>0 || $('html').data('backAtBase') == true ) {									
					...
					if ( $('html').data("lastStand") == 'standing') {						
						...
						console.log("TO - last stand fallen, und back at base = true");
						return;
						}	
					console.log("JQM hash-changePage fires to "+to);					
				...
				if (!$panels>0 || $('html').data('backAtBase') == true ) {										
					if ( $('html').data("lastStand") == 'standing') {						
						$('html').data("lastStand", "fallen");	
						console.log("last stand fallen, und back at base = true");
						return;
						}					
					console.log("no TO - JQM-hash-first page changePage fires to"+$.mobile.firstPage);					
		...
		doneFunc = function() {
			...
			if ( $from && $from[ 0 ] !== $to[ 0 ] ) {						
				...
				if ( $to.parents('.ui-page-active').length == 1  && $from.parents('.ui-page-active').length == 1 ) {
							// console.log("A clearing="+$from.attr('id') );
							$from.removeClass( $.mobile.activePageClass ) 
							} else { 							
								// console.log("B clearing from.closestwrap ="+$from.closest(':jqmData(wrapper="true")').attr('id')+" and self="+$from.attr('id') );
...
$.widget( "mobile.collapsibleset", $.mobile.widget, {
	options: {
		// XXX FREQUENT - add grid
		grid: null,
		initSelector: ":jqmData(role='collapsible-set')"
	},
	_create: function() {
		var $el = this.element.addClass( "ui-collapsible-set" ),			
			// XXX FREQUENT - add direction and top corners
			o = $.extend({ direction: $el.jqmData("type") }, this.options ),		
			flCorners = o.direction == "horizontal" ? [ "ui-corner-tl", "ui-corner-tr" ] : [ "ui-corner-top", "ui-corner-bottom" ],
		...
		if ( !$el.jqmData( "collapsiblebound" ) ) {
			...
			$el
				...
				.bind( "expand collapse", function( event ) {
					...
					// XXX FREQUENT - if all closed add bottom corners on isCollapse
					if ( o.direction == "horizontal" ) {
						
						var allClosed = $el.find( ".ui-collapsible-collapsed" ).length == $el.find( ".ui-collapsible" ).length;
			...
			collapsiblesInSet.first()
				.find( "a" )
					.first()
					// XXX FREQUENT - swap top and bottom classes for flCorners 
		...
		$.extend( widget, {
			...
			_buildList: function() {
				...
				self.select.find( "option" ).each( function( i ) {
					...
					// XXX FRE - added theme select menu list items

					lis.push( "<li data-" + $.mobile.ns + "option-index='" + i + "' data-" + $.mobile.ns + "theme='"+ widget.options.listItemTheme +"' data-" + $.mobile.ns + "icon='"+ dataIcon +"' class='"+ classes.join(" ") + "' " + extraAttrs.join(" ") +">"+ anchor +"</li>" );					
				});
...
$.widget( "mobile.selectmenu", $.mobile.widget, {
	options: {
		...
		menuPageTheme: "b",
		overlayTheme: "a",
		// XXX - added theme for select menu list items
		listItemTheme: "c",
...
$.mobile.fixedToolbars = (function() {
	...
	var stickyFooter, delayTimer,
		...
		autoHideMode = false,
		showDelay = 100,
		// XXX FREQUENT - add overthrow
		ignoreTargets = "a,input,textarea,select,button,label,.ui-header-fixed,.ui-footer-fixed,.overthrow,.noscroll",
	...
	$.extend($.mobile, {
		...
		initializePage: function() {
			...
			// otherwise, trigger a hashchange to load a deeplink
				...
				if ( $nestedPage.closest('div:jqmData(role="panel")').length > 0 ) {						
					// remember deeplink										
